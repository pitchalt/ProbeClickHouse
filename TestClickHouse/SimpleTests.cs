...
//            string cstr = "Compress=False;BufferSize=32768;SocketTimeout=10000;CheckCompressedHash=False;Compressor=lz4;Host=file-server;Port=9000;Database=default;User=test;Password=123") {
            string cstr = "Compress=False;BufferSize=32768;SocketTimeout=10000;CheckCompressedHash=False;Compressor=lz4;Host=localhost;Port=32769;Database=default;User=default") {
        ...
        public void DecimalParam() {
            using (var cnn = GetConnection()) {
                cnn.CreateCommand("DROP TABLE IF EXISTS decimal_test").ExecuteNonQuery();
                cnn.CreateCommand("CREATE TABLE decimal_test (date DATE, dec1 DECIMAL(13,2), dec2 DECIMAL(13,2), dec3 DECIMAL(13,2), dec4 DECIMAL(13,2), dec5 DECIMAL(13,2)) ENGINE = MergeTree() ORDER BY (date, dec1)").ExecuteNonQuery();
                var cmd = cnn.CreateCommand("insert into decimal_test (date, dec1, dec2, dec3, dec5) values('1970-01-01',@d,@d,@d,@d)");
